...
static TaskHandle_t ws_tx_task_handle = NULL;
...
static async_resp_arg_t rsp_arg = {0};
/*
...
static bool config_server_get_wifi_connected(void)
{
	return WIFI_CONNECTED_BIT == (xEventGroupGetBits(xServerEventGroup) & WIFI_CONNECTED_BIT);
}
...
{
	xdev_buffer ucTX_Buffer;
	char ws_data[DEV_BUFFER_LENGTH * 8];

	httpd_ws_frame_t ws_pkt;  
	memset(&ws_pkt, 0, sizeof(ws_pkt));
	ws_pkt.type = HTTPD_WS_TYPE_TEXT;

	const uint64_t MAX_NON_DELAY_PERIOD_MS = (CONFIG_ESP_TASK_WDT_TIMEOUT_S * 1000) / 4;
	uint64_t next_delay_ms = _tick_get_ms() + MAX_NON_DELAY_PERIOD_MS;

	while(1)
	{
		if (xQueueReceive(*xTX_Queue, &ucTX_Buffer, pdMS_TO_TICKS(10)) != pdTRUE) {
			next_delay_ms = _tick_get_ms() + MAX_NON_DELAY_PERIOD_MS;
			continue;
		}

		if (config_server_get_wifi_connected() && ws_client && ws_authenticated) {
			int offset = 0;
			while (true) {
				memcpy(ws_data + offset, ucTX_Buffer.ucElement, ucTX_Buffer.usLen);
				offset += ucTX_Buffer.usLen;

				if (xQueuePeek(*xTX_Queue, &ucTX_Buffer, pdMS_TO_TICKS(2))
						&& offset + ucTX_Buffer.usLen < sizeof(ws_data)) {
					if (xQueueReceive(*xTX_Queue, &ucTX_Buffer, 0) != pdTRUE) {
			...
			if (esp_websocket_client_send(ws_client, ws_data, offset, pdMS_TO_TICKS(MAX_NON_DELAY_PERIOD_MS)) != offset) {
				ESP_LOGE(TAG, "esp_websocket_client_send() fails");
				assert(false);
			} else {
				ESP_LOGI(TAG, "Send=%.*s", offset, ws_data);
			}

			// prevents 'task_wdt: Task watchdog got triggered. The following tasks did not reset the watchdog in time' problem
			if(_tick_get_ms() > next_delay_ms) {
				vTaskDelay(pdMS_TO_TICKS(1)); 
				next_delay_ms = _tick_get_ms() + MAX_NON_DELAY_PERIOD_MS;

				ESP_LOGW(TAG, "Sleep 1 ms");
			}
		}
...
static const unsigned int REMOTE_TYPE_CLIENT = 0x200;

static const unsigned int g_dongleId = 0x368c7548; // 915174728

// 'ABIT.client' EOL record (REMOTE_TYPE_CLIENT)
// 
// Note: access (REMOTE_TYPE_CLIENT below) is actually ignored and input 'roleType' is used
//
static const uint8_t eolRecords[] = {
	0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // access (16)
	0x41, 0x42, 0x49, 0x54, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // server UIN (16)
	0x63, 0x6C, 0x69, 0x65, 0x6E, 0x74, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // name (32)
	0xB9, 0x0F, 0x5C, 0xC7, 0x46, 0x06, 0xC0, 0x15, 0xB0, 0xB2, 0x0B, 0x2B, 0xDD, 0x08, 0x67, 0x39,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // pk (64)  Customer_AES
	0x08, 0xEF, 0xAB, 0x2F, 0x81, 0xE1, 0x92, 0xFD, 0xF1, 0x38, 0xE3, 0xEC, 0x32, 0xD5, 0x41, 0x3F,
	0x33, 0xF2, 0xF2, 0x31, 0x96, 0x05, 0xC5, 0xFF, 0x2B, 0x71, 0x7A, 0x02, 0x88, 0x49, 0x4C, 0x45, // prk (32) Customer_PrK
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // pk_2(64) NotUsed
};

typedef struct
{
	enum {
		EOL__NUM_ECC_DIGITS = 32, // from ecc256.h
		EOL__AES128_KEY_SIZE = 16, // from aes128.h
	} Waste1;

	enum {
		EOL_RECORD_SIZE = 224,
		EOL_RECORD_MAX_COUNT = 30,

		EOL_SERVER_UIN_LENGTH = 16,
		EOL_NAME_LENGTH = 32,

		EOL_SERVER_UIN_OFFSET = 16,
		EOL_NAME_OFFSET = (EOL_SERVER_UIN_OFFSET + EOL_SERVER_UIN_LENGTH),
		EOL_PK1_OFFSET = (EOL_NAME_OFFSET + EOL_NAME_LENGTH),
		EOL_PRK_OFFSET = (EOL_PK1_OFFSET + EOL__NUM_ECC_DIGITS * 2),
		EOL_PK2_OFFSET = (EOL_PRK_OFFSET + EOL__NUM_ECC_DIGITS),
	} Waste2;
} EolRecord;

static char authBuff[600];

static char* base64urlEncoding(const void* data, const size_t dataLengh)
{
	size_t destLength = 0;
	if (mbedtls_base64_encode(NULL, 0, &destLength, (const unsigned char*)data, dataLengh) != MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL) {
		ESP_LOGE(TAG, "Unexpected mbedtls_base64_encode(0) result");
		return NULL;
	}

	char* outputData = (char*)malloc(sizeof(unsigned char) * (destLength + 1));
	outputData[destLength] = 0; // so that it can be interpreted like null-terminated string

	if (mbedtls_base64_encode((unsigned char*)outputData, destLength, &destLength, (const unsigned char*)data, dataLengh) != 0) {
		ESP_LOGE(TAG, "Unexpected mbedtls_base64_encode(%d) result", destLength);
		free(outputData);
		return NULL;
	}
	...
	// beautifies for JWT
	char* ch = NULL;
	while ((ch = strchr(outputData, '+'))) {
		*ch = '-';
	}
	while ((ch = strchr(outputData, '/'))) {
		*ch = '_';
	}
	for (int i = (int)strlen(outputData) - 1; i > 0; --i) {
		if (outputData[i] == '=') {
			outputData[i] = 0;
		} else {
			break;
...
static void reverse_buff(void* data, int data_len)
{
	uint8_t* buff = (uint8_t*) data;

	for (int i = 0; i < data_len / 2; ++i) {
		const uint8_t oldValue = buff[i];
		buff[i] = buff[data_len - i - 1];
		buff[data_len - i - 1] = oldValue;
	}
}

static bool ws_server_authenticate(esp_websocket_client_handle_t client, const char* data, const int data_len)
{
	ESP_LOGI(TAG, "Authentication started..");
	unsigned char digest[32];

	// gets 'timestamp' from input
	memcpy(digest, data, MIN(data_len, sizeof(digest) - 1));
	digest[MIN(data_len, sizeof(digest) - 1)] = 0;
	const uint64_t timestamp = strtoull((const char*) digest, NULL, 10);

	// initialize JWT fields
	time_t seconds = time(NULL);

	esp_fill_random(authBuff, 16);
	char* nonceEnc = base64urlEncoding(authBuff, 16);
	if (nonceEnc == NULL) {
		return false;
	}

	int realSize = snprintf(authBuff, sizeof(authBuff), payload, g_dongleId, (int64_t)seconds, nonceEnc, REMOTE_TYPE_CLIENT, timestamp);
	free(nonceEnc);

	if (realSize <= 0 || realSize >= sizeof(authBuff)) {
		ESP_LOGE(TAG, "JWT payload buffer insufficient (%d)", realSize);
		return false;
	}

	char* headerEnc = base64urlEncoding(header, sizeof(header));
	char* payloadEnc = base64urlEncoding(authBuff, strlen(authBuff));
	if (headerEnc == NULL || payloadEnc == NULL) {
		return false;
	}

	authBuff[0] = 0;
	strcat(authBuff, headerEnc);
	strcat(authBuff, ".");
	strcat(authBuff, payloadEnc);

	free(headerEnc);
	free(payloadEnc);

	int result = mbedtls_sha256((const unsigned char*) authBuff, strlen(authBuff), digest, 0); // '0' for SHA-256
	if (result != 0) {
		ESP_LOGE(TAG, "mbedtls_sha256() fails, reason = %d", result);
		return false;
	}
	reverse_buff(digest, sizeof(digest));

	uint8_t* privateKey = (uint8_t*)(eolRecords + EOL_PRK_OFFSET);
	unsigned char signature[EOL__NUM_ECC_DIGITS * 2];

	int tries_count = 5;
	while (--tries_count > 0) {
		unsigned char random[EOL__NUM_ECC_DIGITS];
		esp_fill_random(random, sizeof(random));

		if (ecdsa_sign(signature, signature + EOL__NUM_ECC_DIGITS, privateKey, random, digest) != 1) {
			ESP_LOGE(TAG, "ecdsa_sign() fails, repeat");
		} else {
			break;
		}
	}
	reverse_buff(signature, EOL__NUM_ECC_DIGITS);
	reverse_buff(signature + EOL__NUM_ECC_DIGITS, EOL__NUM_ECC_DIGITS);
	
	char* signatureEnc = base64urlEncoding(signature, sizeof(signature));
	if (signatureEnc == NULL) {
		return false;
	}

	strcat(authBuff, ".");
	strcat(authBuff, signatureEnc);
...
static void websocket_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case WEBSOCKET_EVENT_DATA:
		if (data->op_code == WS_TRANSPORT_OPCODES_PING) {
			ESP_LOGW(TAG, "Received PING...");
			break; // will be handled by websocket_client implementation
		} else if (data->op_code == WS_TRANSPORT_OPCODES_PONG) {
			ESP_LOGW(TAG, "Received PONG");
			break; // will be handled by websocket_client implementation
		...
		if (data->op_code == WS_TRANSPORT_OPCODES_TEXT || data->op_code == WS_TRANSPORT_OPCODES_BINARY) {
			if (!ws_authenticated) {
				assert(data->data_len == data->payload_len);

				ws_authenticated = ws_server_authenticate(data->client, data->data_ptr, data->data_len);
			} else {
				static xdev_buffer rx_buffer;
				rx_buffer.dev_channel = DEV_WIFI_WS;

				const int totalLength = data->data_len;
				int offset = 0;

				while (offset < totalLength) {
					rx_buffer.usLen = MIN(totalLength - offset, sizeof(rx_buffer.ucElement));
					memcpy(rx_buffer.ucElement, data->data_ptr + offset, rx_buffer.usLen);
					if (xQueueSend( *xRX_Queue, &rx_buffer, portMAX_DELAY ) != pdTRUE) {
...
static void ws_server_task_reset(esp_websocket_client_handle_t client)
{
	static const TickType_t WEBSOCKET_RECONNECT_TIMEOUT = pdMS_TO_TICKS(5*1000);

	if (client) {
		const esp_err_t resCode = esp_websocket_client_destroy(client);
		if (resCode != ESP_OK) {
			ESP_LOGE(TAG, "esp_websocket_client_destroy() fails = %d", (int) resCode);
		}
	}

	vTaskDelay(WEBSOCKET_RECONNECT_TIMEOUT);
}

static void ws_server_rx_task(void *pvParameters)
{
wait_wifi_station:
	xEventGroupWaitBits(
						xServerEventGroup,   /* The event group being tested. */
	...
	ESP_LOGI(TAG, "WebSocket connecting to %s...", websocket_cfg.uri);

    esp_websocket_client_handle_t client = esp_websocket_client_init(&websocket_cfg);
    if (client == NULL) {
		ESP_LOGE(TAG, "esp_websocket_client_init() fails");
		ws_server_task_reset(client);
		goto wait_wifi_station;
	}

	esp_err_t resCode = esp_websocket_register_events(client, WEBSOCKET_EVENT_ANY, websocket_event_handler, (void *)client);
    if (resCode != ESP_OK) {
		ESP_LOGE(TAG, "esp_websocket_register_events() fails = %d", (int) resCode);
		ws_server_task_reset(client);
		goto wait_wifi_station;
	}

    resCode = esp_websocket_client_start(client);
    if (resCode != ESP_OK) {
		ESP_LOGE(TAG, "esp_websocket_client_start() fails = %d", (int) resCode);
		ws_server_task_reset(client);
		goto wait_wifi_station;
	}
	...
	while (true) {
		...
		if (!config_server_get_wifi_connected()) {
			ws_client = NULL;

			ESP_LOGW(TAG, "WiFi disconnected");

			resCode = esp_websocket_client_close(client, pdMS_TO_TICKS(200));
			if (resCode != ESP_OK) {
				ESP_LOGE(TAG, "esp_websocket_client_close() fails = %d", (int) resCode);
			}
