...
static void ws_tx_task(void *pvParameters)
{
	...
	while(1)
	{
		...
		if (config_server_get_wifi_connected() && ws_client && ws_authenticated) {
			...
			const int sentBytes = esp_websocket_client_send(ws_client, ws_data, offset, pdMS_TO_TICKS(50));
			if (sentBytes < 0) {
				ESP_LOGE(TAG, "esp_websocket_client_send() fails");
				assert(false);
			} else {
				//ESP_LOGI(TAG, "Send=%.*s", offset, ws_data);
				//ESP_LOGW(TAG, "Remains %d of %d bytes", (offset - sentBytes), offset);
				bytes_after_delay += sentBytes;
			}
			...
			if(_tick_get_ms() > next_delay_ms) {
				...
				ESP_LOGW(TAG, "Sleep 1 ms , bytes_after_delay = %d", bytes_after_delay);
				bytes_after_delay = 0;
			}
		}
...
static const unsigned int g_dongleId = 0x368C7297; // 915174039
...
static const uint8_t eolRecords[] = {
	...
	0x52, 0x65, 0x6D, 0x6F, 0x74, 0x65, 0x43, 0x6C, 0x69, 0x65, 0x6E, 0x74, 0x31, 0x0A, 0x00, 0x00, // name (32)
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0xB9, 0x0F, 0x5C, 0xC7, 0x46, 0x06, 0xC0, 0x15, 0xB0, 0xB2, 0x0B, 0x2B, 0xDD, 0x08, 0x67, 0x39, // pk (64)  Customer_AES
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	0x08, 0xEF, 0xAB, 0x2F, 0x81, 0xE1, 0x92, 0xFD, 0xF1, 0x38, 0xE3, 0xEC, 0x32, 0xD5, 0x41, 0x3F, // prk (32) Customer_PrK
	0x33, 0xF2, 0xF2, 0x31, 0x96, 0x05, 0xC5, 0xFF, 0x2B, 0x71, 0x7A, 0x02, 0x88, 0x49, 0x4C, 0x45,
